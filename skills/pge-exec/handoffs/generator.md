...
Generator owns implementation, proportional issue verification, self-review, and evidence production for its assigned issue candidate. It should start work when dispatched and must not wait for a fixed per-issue Evaluator hop before doing implementation work.
...
**Hot-path size rule:** The execution brief is intentionally small. It carries the issue goal, semantic plan context, change, target areas, recommended approach, forbidden boundaries, and validation. Do not ask main for the full plan, all issue files, or the full Candidate Gate checklist before starting. Detailed quality audits are produced in `generator_completion` and governed by `references/generator-rules.md`.
...
Goal: <issue-local goal>
Plan Context: <semantic plan intent, decision, phase, or slice>
Change: <bounded issue-local change to deliver>
Target Areas: <exact file paths — Create: X | Modify: Y>
Recommended Approach: <implementation direction>
Forbidden: <boundaries this issue must not cross>
Validation:
  Expected: <what must be true>
  Check: <command, inspection, or manual check>
  Evidence: <what pge-exec should record>

## Optional Risk-Triggered Context

Stop If: <only if high-risk or boundary-sensitive>
Verification Coupling: <only if non-independent or shared verification matters>
Prior Issues: <only if dependency/coupling matters>
Risk Checks: <security / public API / schema / persistence / performance / route vocabulary / none>
Prep Hint: <read-only prep conclusions, or "none">

## Rules

1. Execute the issue change. Produce the required deliverable or equivalent output.
2. Fresh-read the current code reality around Target Areas and Plan Context before editing. If it conflicts with the brief in a way that changes goal, forbidden boundaries, target areas, change, or validation, report `BLOCKED` with the contract gap and the clarification needed. Do not silently rewrite the contract.
3. Use a proportional feedback loop per Validation and Change. For behavior changes, prefer meaningful RED → GREEN evidence. For schema/config/docs/mechanical contract changes, use the plan's explicit validation check or strongest contract-level check instead of inventing low-value tests.
4. Run Validation. Record output as evidence.
5. Use Implementation Guidance only as issue-specific shaping. Do not treat prep hints as evidence. Re-check current code reality before relying on them.
6. Run the Candidate Quality Gate: goal alignment, forbidden-boundary check, target-area check, validation/evidence check, and any risk-triggered checks named in the brief. This is not reflective self-approval; inspect concrete evidence. Fix local in-contract findings before completion; report BLOCKED if a finding cannot be fixed inside the issue contract.
...
8. Report execution-time implementation notes for meaningful decisions, tradeoffs, deviations, blockers, follow-ups, learned constraints, open questions, or verification gaps that the plan did not spell out. Use `implementation_notes: none` only when no such notes exist.
...
- TDD evidence quality: tests must verify issue behavior through a public or plan-relevant interface and be proportional to validation. Do not add tests that simply restate the implementation, assert private structure, mirror the code path, or check only that a mocked collaborator was called; these provide zero confidence. If no meaningful RED test is possible, record why and use contract-level verification.
- Deviation classification:
  ...
  - contract-blocked: new service, schema change, scope expansion, public API change, user decision, package install failure, or plan contract ambiguity → report BLOCKED with the contract reason and what clarification or RD/user decision would restore execution
...
- Scope boundary: fix only what the issue change and validation require. Adjacent in-contract issue-boundary adjustments need notes; unrelated → deferred items.
...
Completion is a compact result packet. Do not emit a full audit table by default. Add risk-triggered evidence only when the issue touched security, public API/CLI, schema/artifact layout, persistence/migration, performance-sensitive code, route/state/vocabulary, or other DEEP/cross-module behavior.
...
changed_files: <list>
validation: <validation check/result/evidence>
deviations: <any deviations from plan, or "none">
notes: <meaningful implementation decision, tradeoff, follow-up, or "none">
risk_triggered_evidence: <additional caller/security/schema/performance/simplification evidence when triggered, or "none">
...
Change: <original issue change>
Target Areas: <original Target Areas>
Validation: <original validation>
Verification Coupling: <original plan-index / run-state coupling>

## Rules

1. Fix ONLY what required_fixes specifies.
2. Do not broaden scope.
3. Re-run Validation. Record output.
4. Preserve the original goal, plan context, change, target areas, recommended approach, forbidden boundaries, and validation.
5. Send a fresh compact `generator_completion`; include risk-triggered evidence only when the repair touches a triggered risk surface.
...
- Validation evidence is present in the completion message
...
- Risk-triggered evidence is present only when the issue/repair touches a triggered risk surface
- TDD / verification evidence is proportional to validation and does not rely on implementation-restating tests
