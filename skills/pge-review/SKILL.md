...
When a matching `.pge/tasks-<slug>/research.md` exists beside the plan, read the current `research.v3` fields (`goal`, `success_shape`, `scope`, `non_goals`, `constraints`, relevant context, assumptions, `simplest_direction`, `rejected_directions`, open questions, conditional Implementation Friction, conditional Progressive Feasibility, and `route`) only as needed to detect semantic drift between original intent, plan, and diff. Do not recover review intent from obsolete Research-era field names; current `research.v3`, the current plan, and the current prompt outrank historical Research artifacts.
