...
When a matching `.pge/tasks-<slug>/research.md` exists beside the plan, read the current `research.v3` fields (`goal`, `success_shape`, `scope`, `non_goals`, `constraints`, relevant context, assumptions, `simplest_direction`, `rejected_directions`, open questions, conditional Implementation Friction, conditional Progressive Feasibility, and `route`) only as needed to detect semantic drift between original intent, plan, and diff. Legacy `research.v2` fields (`intent_framings`, `confirmed_intent`, `scope_contract`, `success_shape`, `experience_scope`, conditional `design_surface_context`, `upstream_contract`, `evidence`, `reality_alignment_proof`, `ambiguities`, `interactive_alignment`, `planning_handoff`, `route`) remain compatibility input. Older fields such as `intent_spec`, `plan_delta`, and `blockers` are compatibility input only; do not treat them as stronger than the current plan/research contract.
