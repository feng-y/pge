...
When a matching `.pge/tasks-<slug>/research.md` exists beside the plan, read the current `research.v3` fields (`goal`, `success_shape`, `scope`, `non_goals`, `constraints`, relevant context, assumptions, `simplest_direction`, `rejected_directions`, open questions, conditional Implementation Friction, conditional Progressive Feasibility, and `route`) only as needed to detect semantic drift between original intent, plan, and diff. Older Research terms such as `research.v2`, `confirmed_intent`, `scope_contract`, `planning_handoff`, and `plan_delta` are obsolete or foreign-source evidence only; do not treat them as active Research compatibility surfaces or stronger than the current plan/research contract.
