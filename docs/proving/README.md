...
Do not treat `claude -p "/pge-execute test"` as proof of the Agent Team runtime path by itself. The proving rule is direct execution: run the real `/pge-execute ...` command in the target Claude surface, let it attempt the current Team control-plane calls, and if a runtime call fails, keep the concrete call error as the proving result instead of replacing it with a pre-check.
...
For a visible runtime proof, use a Claude Code runtime surface that can execute the Agent Team control-plane calls, then run `/pge-execute test` there after reinstall/reload. A `claude -p` invocation is only a load check unless that runtime is authenticated and explicitly exposes those Agent Team calls.
