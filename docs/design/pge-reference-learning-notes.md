...
1. **Contract 协商机制** — Generator 提案 → Evaluator 审查 → 迭代 → 达成 contract → 实现。通信通过文件交换，不是对话。PGE 的 preflight 阶段已经实现了这个模式的雏形，但可以强化迭代协商的结构。
   - 当前 repo 注记：`handoffs/preflight.md` 已退出 active executable lane，这里描述的是 PGE 的 future / archived seam，不是当前主链行为。
...
| Contract 协商强化 | `skills/pge-execute/handoffs/preflight.md` — archived / future 协商结构 |
...
| 双格式交接 | `skills/pge-execute/handoffs/` — future / recovery 交接面 |
...
**PGE 映射**: archived preflight 设计（`skills/pge-execute/handoffs/preflight.md`）曾经承担这个模式；当前 active lane 先收敛到 planner/generator/evaluator 三段主链。可以把 anti-pattern 防护和自审 checklist 直接强化到 planner/generator 上。
