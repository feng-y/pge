...
2. **Hard Threshold 评分** — Evaluator 对每个评估维度设硬阈值，任一维度低于阈值则 sprint 失败。开箱即用的 Claude 是糟糕的 QA agent（会识别问题然后说服自己"不是大问题"），必须用硬阈值对抗这种倾向。
...
| 设计点 | 目标模块 |
|--------|---------|
| Hard Threshold 评分 | `agents/pge-evaluator.md` — 增加量化评分维度和硬阈值 |
...
| Evaluator 校准 | `skills/pge-execute/contracts/evaluation-contract.md` — 增加校准 fixtures 定义 |
...
### 4. 量化而非叙述（Quantify, Don't Narrate）

多个项目要求用量化指标替代叙述性判断：
- **Anthropic**: Hard Threshold 评分 + few-shot 校准
- **gstack**: Confidence Calibration（1-10 分）+ Completeness Score（X/10）
- **Best Practice**: Instruction Budget（<40 条/步骤）+ Context Rot 阈值（<30%）

**PGE 映射**: 当前 Evaluator 使用叙述性判断。需要在 `skills/pge-execute/contracts/evaluation-contract.md` 中增加量化评分维度。
