...
- Preflight 协商默认由 Generator 和 Evaluator 直接通信
- Planner / Generator / Evaluator 之间的常规反馈优先走 `SendMessage`
- 文件只记录“阶段收敛结果”，不记录“讨论过程本身”
- Resume 依赖 checkpoint / state / handoff，不依赖旧消息历史
...
| `mode_decision` | evaluator → main | 说明 `FAST_PATH / LITE_PGE / FULL_PGE / LONG_RUNNING_PGE` |
| `preflight_verdict` | evaluator → main | 给出 PASS/BLOCK/ESCALATE |
...
| `state.json` | 机器可读 source of truth |
| `checkpoint.json` / `handoff.md` | 恢复与跨阶段延续 |
| `summary.md` / `progress.md` | mode-required human-readable 派生物 |
