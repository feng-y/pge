...
> 调研日期: 2026-04-29 (updated)
> 主参考: "Harness design for long-running application development" (2026-03-24, Prithvi Rajasekaran)
> 来源: Anthropic Engineering Blog 系列文章
> 环境: Claude Agent SDK (Python code orchestration)，非 Claude Code Agent Teams
...
> "I created a planner agent that took a simple 1-4 sentence prompt and expanded it into a full product spec. I prompted it to be ambitious about scope and to stay focused on **product context and high level technical design** rather than detailed technical implementation."

> "This emphasis was due to the concern that if the planner tried to specify granular technical details upfront and got something wrong, the errors in the spec would cascade into the downstream implementation. It seemed smarter to **constrain the agents on the deliverables to be produced** and let them figure out the path as they worked."

**关键点**: Planner 只定义 what（产品 spec），不定义 how。不做 mode selection、不判断复杂度、不决定是否跳过步骤。
...
每个 sprint 开始前，Generator 和 Evaluator 协商一个 **sprint contract**（原文）：
...
- 双方迭代直到达成一致
- **Planner 不参与 preflight** — Planner 只产出高层 spec，contract 协商是 G↔E 之间的事
...
**环境约束**: Anthropic 的 harness 运行在 **Claude Agent SDK**（Python 代码编排）上，没有 Agent Teams 的 SendMessage 原语。文件通信是 SDK 环境下的自然选择，不是架构偏好。

这个机制存在是因为产品 spec 故意保持高层级，需要一个步骤来桥接用户故事和可测试的实现。

### V2 简化: 移除 Sprint 构造

在 Opus 4.6 上，sprint 构造被完全移除（原文）：
...
- Generator 连续工作 2+ 小时无需 sprint 分解
- SDK 自动 compaction 处理 context 增长
...
**关键洞察**: Evaluator 不是固定的 yes/no 决策。当任务超出当前模型独立可靠完成的范围时，它才值得投入成本。这直接对应 PGE P0-1 的 Execution Cost Gate 概念——由 Evaluator 判断是否需要完整评估流程。
...
**通信方式**: 文件交换。一个 agent 写文件，另一个读取并在该文件中回应或创建新文件。（见上文 §3 原文引用）

**环境注意**: 这是 Claude Agent SDK 的约束——SDK 没有 Agent Teams 的 SendMessage。PGE 运行在 Claude Code Agent Teams 上，可以用 direct messaging 做 live negotiation，只把 locked results 落文件。见 `pge-agent-teams-communication-design.md`。
...
> "Out of the box, Claude is a poor QA agent. In early runs, I watched it identify legitimate issues, then **talk itself into deciding they weren't a big deal** and approve the work anyway. It also tended to test superficially."
...
- **Planner 不参与 contract 协商** — 只产出高层 spec
- 通信通过文件（SDK 环境约束，非架构偏好）
- PGE 在 Agent Teams 环境下可用 SendMessage 做 live negotiation
- Contract 包含可测试的验证标准（Sprint 3 有 27 个标准覆盖 level editor）
...
对比 Solo（无 harness）：20 min / $9，但核心功能不工作。Harness 20x 成本换来可用的产品。

### 11.9 End-to-End 测试不可替代

Claude 倾向于标记 feature 为完成但未做端到端测试。提供浏览器自动化工具（Playwright MCP）后性能显著提升。Agent 能识别和修复仅从代码看不出的 bug。

## 12. Claude Agent SDK vs Claude Code Agent Teams

Anthropic 的 harness 运行在 **Claude Agent SDK**（Python 代码编排），PGE 运行在 **Claude Code Agent Teams**。两者的能力差异直接影响 PGE 的设计决策。

### 12.1 能力对比

| 能力 | Agent SDK | Agent Teams | PGE 影响 |
|------|-----------|-------------|----------|
| **Agent 间通信** | 只有文件系统 | SendMessage（direct messaging） | PGE 可用 live messaging 做 negotiation，不必所有交互都写文件 |
| **编排控制** | Python 代码，完全可编程 | Prompt-driven，Claude 解释执行 SKILL.md | PGE 的 orchestration 不是确定性的，依赖 Claude 正确理解指令 |
| **状态机** | Python 代码实现，确定性 | Markdown 规则 + JSON 文件，Claude 解释 | PGE 的状态转换可能出错，SDK 的不会 |
| **Agent 生命周期** | 代码控制创建/销毁 | TeamCreate/Agent/TeamDelete API | PGE 受限于 Agent Teams API 的能力边界 |
| **Context 管理** | SDK 自动 compaction，可编程 reset | Claude Code 自动 compaction | PGE 无法像 SDK 那样精确控制 context reset 时机 |
| **并行执行** | 可编程并行 subagent | Agent Teams 支持并行 teammate | 能力相当 |
| **工具访问** | 可定制工具集 per agent | Agent 定义中的 tools 列表 | PGE 可以限制每个 agent 的工具权限 |
| **持久状态** | 代码直接读写文件/DB | 文件系统 + git | 能力相当 |
| **错误恢复** | try/catch，代码级别 | 无结构化错误处理 | PGE 的 error recovery 依赖 Claude 判断，不是代码保证 |
| **成本控制** | 可编程 token budget | 无直接 token 预算控制 | PGE 无法像 SDK 那样设 per-agent token limit |

### 12.2 PGE 的优势

- **Direct messaging**: Agent Teams 的 SendMessage 让 G↔E negotiation 可以用 live messages 而不是文件轮转，减少 artifact 数量
- **Native team structure**: TeamCreate 原生支持 team membership，不需要代码编排
- **Shared context**: 同一 team 内的 agents 共享文件系统访问

### 12.3 PGE 的劣势

- **非确定性编排**: SKILL.md 指令由 Claude 解释执行，不是代码保证。状态机可能走错路径
- **无精确 context 控制**: 无法编程式 reset context window 或设置 token budget
- **无结构化错误处理**: 没有 try/catch，agent 出错时的恢复依赖 prompt 指令
- **Gate 检查是 prompt 级**: 只能检查 section 是否存在，不能做代码级的内容验证

### 12.4 对 PGE 设计的影响

1. **通信设计**: 利用 SendMessage 做 live negotiation 是正确方向（P0-2），Anthropic 用文件是 SDK 的约束不是架构偏好
2. **可靠性**: PGE 需要更强的 gate 检查和 artifact 验证来补偿非确定性编排的风险
3. **成本控制**: PGE 的 adaptive execution（P0-1 Execution Cost Gate）部分补偿了无法设 token budget 的问题——由 Evaluator 在 triage 阶段决定投入多少资源
4. **Harness 简化**: Anthropic V2 移除 sprint 后 Evaluator 做单次评估的模式，对应 PGE 的 FAST_PATH / LITE_PGE mode——简单任务不需要重流程
