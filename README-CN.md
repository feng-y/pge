...
PGE 也可以采纳其他工作流产生的计划。如果 Claude 计划模式输出、`docs/exec-plan/` 文档或外部工作流计划清晰完整——目标、范围、语义所有权、非目标、目标区域或所有权边界、实现方向和验证/证据检查点都存在——`pge-plan` fast-adopt 会将其转换为 `.pge/tasks-<slug>/plan.md`，并在允许执行前运行 Final Plan Gate。采纳后，`pge-exec` 只在 `plan_gate` 通过时消费该规范产物。
...
`pge-ai-native-refactor`、`pge-handoff`、`pge-knowledge`、`pge-html`、`pge-complexity`、`pge-diagnose`、`pge-grill-me`、`pge-redo` 和 `pge-zoom-out` 是支持界面。它们在弧线周围很有用，但不替代主阶段合约。
...
- **[`/pge-plan`](./skills/pge-plan/SKILL.md)** — 在 `.pge/tasks-<slug>/plan.md` 下产生经过工程审查的有界计划。将意图转化为带有验收标准、验证提示、证据要求、禁止区域、仓库现实检查和 Final Plan Gate 的编号可执行议题合约；Final Plan Gate 通过前不能进入 `pge-exec`。
...
- **[`/pge-html`](./skills/pge-html/SKILL.md)** — 将规范 PGE artifacts 渲染为保真的单文件 HTML 页面和派生决策板。保真页面保留源结构；决策板把 artifacts 压缩为 issue、evidence、risk、gate 和 human-attention 视图，同时保持 Markdown/JSON/evidence 作为事实源。也支持非 PGE 的认知、展示和本地编辑器 HTML artifacts。
