...
**The fix:** PGE structures work into a pipeline with explicit artifacts at each boundary. Research produces findings. Planning produces a contract. Execution consumes that contract issue-by-issue with built-in verification. Each phase is a separate skill invocation — you stay in control of when to advance.
...
Each skill produces an artifact that the next skill consumes. You can enter at any point — skip research if you already know the landscape, skip planning if you already have a plan file.
